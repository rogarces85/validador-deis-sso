...
description: "Reestructura y agrupa las validaciones de reglas_finales.json discriminando por el tipo de establecimiento."
---

# Agrupador de Validaciones por Establecimiento

Esta habilidad se encarga de analizar, clasificar y agrupar las reglas de validación definidas en `data/reglas_finales.json`, discriminándolas según el tipo de establecimiento de salud (ej. Hospitales, CESFAM, Postas, SAPU, SAMU, etc.). Dado que existen reglas específicas para ciertos tipos de atención, esta habilidad permite organizar la lógica de exclusión o inclusión.

## Prerrequisitos
- El archivo de reglas base en `data/reglas_finales.json`.
...
1. Lee el contenido de `data/reglas_finales.json` (`view_file`).
...
1. Para cada validación en `reglas_finales.json`, determina a qué `tipo_establecimiento` o `tipos_establecimiento` aplica.
...
1. Reescribe el archivo `reglas_finales.json` añadiendo la semántica de "Tipo de Establecimiento" y "Establecimientos Específicos" (`aplicar_a`) a las validaciones.
...
1. Busca en el código fuente dónde se procesa `reglas_finales.json`.
