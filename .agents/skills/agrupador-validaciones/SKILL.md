...
Esta habilidad se encarga de analizar, clasificar y agrupar las reglas de validación definidas en `data/Rules_nuevas.json`, discriminándolas según el tipo de establecimiento de salud (ej. Hospitales, CESFAM, Postas, SAPU, SAMU, etc.). Dado que existen reglas específicas para ciertos tipos de atención, esta habilidad permite organizar la lógica de exclusión o inclusión.

## Prerrequisitos
- El archivo de reglas base en `data/Rules_nuevas.json`.
...
1. Lee el contenido de `data/Rules_nuevas.json` (`view_file`).
...
1. Para cada validación en `Rules_nuevas.json`, determina a qué `tipo_establecimiento` o `tipos_establecimiento` aplica.
...
1. Reescribe el archivo `Rules_nuevas.json` añadiendo la semántica de "Tipo de Establecimiento" y "Establecimientos Específicos" (`aplicar_a`) a las validaciones.
...
1. Busca en el código fuente (por ejemplo, en las clases PHP del validador) dónde se procesa `Rules_nuevas.json`.
