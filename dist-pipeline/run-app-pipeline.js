// tests-pipeline/run-app-pipeline.ts
...
var FilenameValidatorService = class _FilenameValidatorService {
  static {
    ...
    this.REGEX_FORMAT = /^(\d{6})([A-Z]{1,2})(\d{2})\.(xlsx|xlsm)$/i;
  }
  static {
    // js-set-map-lookups: O(1) lookup for valid series
    this.SERIES_SET = new Set(VALID_SERIES.map((s) => s.toUpperCase()));
  }
  validate(filename) {
    const errors = [];
...
async function procesar(file) {
  ...
  for (const sheetName of Object.keys(reglas_finales_default)) {
    if (sheetName.startsWith(serie)) {
      const arr = reglas_finales_default[sheetName];
      if (Array.isArray(arr)) applicableRules.push(...arr);
    }
  }
  const ruleEngine = new RuleEngineService();
  const establishment = establishments_catalog_default.establecimientos.find((e) => e.codigo === codigo);
  ...
  const hallazgos = ruleResults.filter((r) => r.resultado === false);
...
async function main() {
  ...
  const filesToProcess = targets.length > 0 ? targets : (await fs.readdir(".")).filter((f) => f.startsWith("123010P06_") && f.endsWith(".xlsm")).sort();
  const resumen = [];
  ...
  const archivosValidados = resumen.filter((r) => !r.errorBloqueante && !r.errorTecnico).length;
  const archivosBloqueados = resumen.filter((r) => r.errorBloqueante).length;
  const archivosTecnicos = resumen.filter((r) => r.errorTecnico).length;
  const conHallazgos = resumen.filter((r) => r.hallazgos && r.hallazgos.length > 0).length;
  const sinHallazgos = resumen.filter((r) => r.hallazgos && r.hallazgos.length === 0 && !r.errorBloqueante).length;
  const totalHallazgos = resumen.reduce((acc, r) => acc + (r.hallazgos?.length || 0), 0);
  const totalReglas = resumen.reduce((acc, r) => acc + (r.reglasEjecutadas || 0), 0);
  const totalAprobadas = resumen.reduce((acc, r) => acc + (r.aprobadas || 0), 0);
  console.log("\n=== RESUMEN DE PIPELINE ===");
